...
Edit `node_modules/parcel-bundler/src/utils/config.js` and patch `resolve`:
...
Edit `node_modules/parcel-bundler/src/Bundler.js` and patch:
...
class Bundler extends EventEmitter {
  ...
  async start() {
    ...
    if (this.options.watch) {
      ...
      this.pathWatcher = new FSWatcher({
        useFsEvents: process.env.NODE_ENV !== 'test'
      });
      this.pathWatcher.on('change', this.onPathChange.bind(this));
    }
  ...
  async buildQueuedAssets(isInitialBundle = false) {
    ...
    while (this.buildQueue.size > 0) {
      ...
      for (let asset of this.buildQueue) {
        ...
        if (!isInitialBundle) {
          await this.invalidateDependencies(asset, loadedAssets);
          asset.invalidate();
          if (this.cache) {
  ...
  async invalidateDependencies(asset, loadedAssets) {
    for (let dep of asset.dependencies.values()) {
      // Invalidate virtual dependencies
      if (dep.name.startsWith('!')) {
        const assetDep = this.loadedAssets.get(dep.name);
        if (assetDep) {
          assetDep.invalidate();
          loadedAssets.add(assetDep);
        }
      }
    }
  }
  ...
  async resolveAsset(name, parent) {
    ...
    if (this.watcher && !path.startsWith('!')) {
  ...
  async loadAsset(asset) {
    ...
    if (asset.paths) {
      ...
      asset.watcher = new FSWatcher();
      asset.watcher.add(asset.paths);
      asset.watcher.on('change', path => {
        console.log('ASSET CHANGE', asset.name, path);
        asset.watcher.close();
        this.reloadAsset(asset);
      });
    }
...
For easier debugging, edit `WorkerFarm.js` to only use the `localWorker`.
